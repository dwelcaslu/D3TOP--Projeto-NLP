...
def remove_stopwords(tokenized_text, stopwords=stopwords.words('english')):
...
def clean_text_stem(text, to_lowercase=True, punctuation=string.punctuation, strip_accents=True,
                    stemmer=nltk.stem.RSLPStemmer(), stopwords=stopwords.words('english'), ngram_range = (1, 3)):
    # Text cleaning:
...
def clean_text_lemmatize(text, to_lowercase=True, punctuation=string.punctuation, strip_accents=True,
                         lemmatizer=nltk.WordNetLemmatizer(), stopwords=stopwords.words('english'), ngram_range = (1, 3)):
    # Text cleaning:
    clean_text = preprocess_text(text, to_lowercase, punctuation, strip_accents)
    if strip_accents:
        stopwords = [unidecode.unidecode(word) for word in stopwords]
...
# Words Essence:
class WordsEssence:
    def __init__(self, max_features=10, ngram_range=(1, 1), stopwords=stopwords.words('english')):
